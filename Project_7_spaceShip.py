# http://www.codeskulptor.org/#user42_FXVXuCtshb_3.py
...
class Ship:
    ...
    def draw(self,canvas): 
        ...
        if self.thrust:
            self.vel[0] += 0.15*forward[0]
            self.vel[1] += 0.15*forward[1]
    ...
    def shoot(self):
        global a_missile
        forward = angle_to_vector(self.angle)
        a = self.pos[0]+self.radius*forward[0]
        b = self.pos[1]+self.radius*forward[1]
        pos = [a,b]
        vel = [self.vel[0]+4*forward[0],self.vel[1]+4*forward[1]]
        
        a_missile = Sprite(pos, vel, 0, 0, missile_image,\
...
def rock_spawner():
    ...
    global a_rock
    w, h = random.random()*800/1, random.random()*600/1
    vel = [random.random()*1-.5,random.random()*1-.5]
    ang = random.random()*math.pi*2
    ang_vel = random.random()*.1 - .05
    a_rock = Sprite([w,h], vel, ang, ang_vel, asteroid_image, asteroid_info)
